...
import { Dimensions, Image, ImageBackground, KeyboardAvoidingView, Modal, Platform, Pressable, ScrollView, StyleSheet, Switch, Text, TextInput, View } from "react-native";
...
export default function TripDetail() {
  ...
  const { getTripById, addExpense, addPhoto, addNote, updateNote, deleteNote, updateTripVisibility, updateTripPrivacy, togglePhotoPrivacy, toggleExpensePrivacy } = useTrips();
  ...
  const [noteTitle, setNoteTitle] = useState(""); const [noteBody, setNoteBody] = useState(""); const [noteMood, setNoteMood] = useState("😊"); const [noteColor, setNoteColor] = useState("#FFF3E0"); const [notePrivate, setNotePrivate] = useState(false);
  ...
  const pickPhoto = async () => {
    ...
    if (!r.canceled) { setTempUri(r.assets[0].uri); setPhotoModal(true); }
  };
  ...
  const openNoteEditor = (note?: any) => {
    if (note) { setEditNote(note); setNoteTitle(note.title); setNoteBody(note.body); setNoteMood(note.mood); setNoteColor(note.color); setNotePrivate(!!note.isPrivate); }
    else { setEditNote(null); setNoteTitle(""); setNoteBody(""); setNoteMood("😊"); setNoteColor("#FFF3E0"); setNotePrivate(false); }
    setNoteModal(true);
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {/* Tab Content */}
      <ScrollView style={styles.flex1} showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 20, paddingBottom: 100 }}>
        {tab === 0 && (
          <View>
        ...
        {tab === 1 && (
          <View>
            <View style={[styles.mapWrap, { borderColor: colors.border }]}>
              {trip.route.length > 0 ? (
                <MapView style={styles.flex1} initialRegion={{ latitude: trip.route[0].lat, longitude: trip.route[0].lng, latitudeDelta: 2, longitudeDelta: 2 }}>
                  <UrlTile urlTemplate="https://basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}@2x.png" maximumZ={19} flipY={false} />
                  <Polyline coordinates={trip.route.map((s) => ({ latitude: s.lat, longitude: s.lng }))} strokeColor={colors.accent} strokeWidth={4} />
                  {trip.route.map((s) => <Marker key={s.id} coordinate={{ latitude: s.lat, longitude: s.lng }}><View style={[styles.mapMarker, { backgroundColor: stopColor(s.color) }]}><Text style={styles.mapMarkerText}>{s.label}</Text></View></Marker>)}
                </MapView>
              ) : <View style={[styles.center, { backgroundColor: colors.surface }]}><Text style={{ color: colors.textMuted }}>No route data</Text></View>}
            </View>
            <Text style={[styles.sectionTitle, { color: colors.text }]}>Detailed Path</Text>
            <View style={[styles.routeCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
              {trip.route.map((s, i) => (
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.expSheetContent}>
          ...
          <View style={styles.expAmtWrap}>
            <View style={[styles.expAmtInput, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              <Text style={[styles.expAmtSign, { color: colors.textMuted }]}>$</Text>
          ...
          {aiPred && aiPred.category !== "Other" && (
            <View style={[styles.aiPredCard, { backgroundColor: colors.aiBadgeBg, borderColor: colors.aiBadgeBorder }]}>
              <Text style={styles.aiPredIcon}>{aiPred.icon}</Text>
      ...
      <Modal visible={noteModal} animationType="slide" presentationStyle="pageSheet">
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={[styles.flex1, { backgroundColor: noteColor }]}>
          ...
          <ScrollView style={styles.noteBody}>
            ...
            <TextInput value={noteBody} onChangeText={setNoteBody} placeholder="What's on your mind?..." placeholderTextColor="rgba(15,23,42,0.3)" style={styles.noteBodyInput} multiline autoFocus={!editNote} />
          </ScrollView>
          <View style={styles.noteToolbar}>
            ...
            <View style={styles.noteMoods}>
              {["😊", "😍", "🤩", "❤️"].map((m) => <Pressable key={m} onPress={() => setNoteMood(m)} style={[styles.noteMoodBtn, noteMood === m && styles.noteMoodActive]}><Text style={styles.noteMoodText}>{m}</Text></Pressable>)}
            </View>
          </View>
        </KeyboardAvoidingView>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  heroName: { color: "#FFF", fontSize: 26, fontFamily: "Inter-Bold", marginRight: 8 },
  heroStatus: { backgroundColor: "rgba(255,255,255,0.2)", paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  heroStatusText: { color: "#FFF", fontSize: 9, fontFamily: "Inter-Bold", textTransform: "uppercase" },
  heroDest: { color: "rgba(255,255,255,0.9)", fontSize: 14, fontFamily: "Inter-Medium", marginBottom: 4 },
  heroDateRow: { flexDirection: "row", alignItems: "center" },
  heroDate: { color: "rgba(255,255,255,0.7)", fontSize: 11, fontFamily: "Inter-Medium", marginLeft: 4 },
  ...
  tabText: { fontSize: 13, fontFamily: "Inter-Bold" },
  statsGrid: { flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", marginBottom: 16 },
  statCard: { width: "48%", marginBottom: 10, padding: 14, borderRadius: 16, borderWidth: 1, alignItems: "center" },
  statIcon: { width: 30, height: 30, borderRadius: 8, alignItems: "center", justifyContent: "center", marginBottom: 6 },
  statValue: { fontSize: 16, fontFamily: "Inter-Bold", marginBottom: 1 },
  statLabel: { fontSize: 9, fontFamily: "Inter-Bold", textTransform: "uppercase", letterSpacing: 0.5 },
  budgetCard: { padding: 16, borderRadius: 16, borderWidth: 1, marginBottom: 20 },
  budgetHeader: { flexDirection: "row", justifyContent: "space-between", marginBottom: 8 },
  budgetTitle: { fontSize: 14, fontFamily: "Inter-Bold" },
  budgetPct: { fontSize: 14, fontFamily: "Inter-Bold" },
  budgetTrack: { height: 5, borderRadius: 3, overflow: "hidden" },
  budgetFill: { height: "100%", borderRadius: 3 },
  sectionTitle: { fontSize: 18, fontFamily: "Inter-Bold", marginBottom: 12 },
  routeCard: { padding: 16, borderRadius: 16, borderWidth: 1, marginBottom: 20 },
  ...
  routeMarkerText: { color: "#FFF", fontSize: 10, fontFamily: "Inter-Bold" },
  routeLine: { width: 2, height: 24 },
  routeInfo: { flex: 1, paddingTop: 2, marginBottom: 12 },
  routeName: { fontSize: 13, fontFamily: "Inter-Bold" },
  routeSub: { fontSize: 10, fontFamily: "Inter-Medium" },
  mapWrap: { height: 260, borderRadius: 16, overflow: "hidden", borderWidth: 1, marginBottom: 16 },
  mapMarker: { width: 30, height: 30, borderRadius: 15, alignItems: "center", justifyContent: "center", borderWidth: 3, borderColor: "#FFF" },
  mapMarkerText: { color: "#FFF", fontSize: 10, fontFamily: "Inter-Bold" },
  pathRow: { flexDirection: "row", alignItems: "center", marginBottom: 12 },
  pathIcon: { width: 36, height: 36, borderRadius: 10, alignItems: "center", justifyContent: "center", marginRight: 12 },
  pathLabel: { fontSize: 11, fontFamily: "Inter-Bold" },
  totalSpentCard: { padding: 28, borderRadius: 20, alignItems: "center", marginBottom: 20 },
  totalSpentLabel: { color: "rgba(255,255,255,0.7)", fontSize: 11, fontFamily: "Inter-Bold", textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 4 },
  totalSpentValue: { color: "#FFF", fontSize: 36, fontFamily: "Inter-Bold" },
  totalSpentRemaining: { backgroundColor: "rgba(255,255,255,0.2)", paddingHorizontal: 12, paddingVertical: 4, borderRadius: 12, marginTop: 12 },
  totalSpentRemainingText: { color: "rgba(255,255,255,0.8)", fontSize: 10, fontFamily: "Inter-Bold" },
  pieCard: { padding: 20, borderRadius: 16, borderWidth: 1, marginBottom: 20 },
  ...
  pieCenterVal: { fontSize: 16, fontFamily: "Inter-Bold" },
  pieCenterLabel: { fontSize: 8, fontFamily: "Inter-Bold", textTransform: "uppercase" },
  pieLegend: { flex: 1, marginLeft: 20 },
  legendRow: { flexDirection: "row", alignItems: "center", marginBottom: 6 },
  legendDot: { width: 8, height: 8, borderRadius: 4, marginRight: 8 },
  legendInfo: { flex: 1, flexDirection: "row", justifyContent: "space-between" },
  legendName: { fontSize: 10, fontFamily: "Inter-Bold" },
  legendPct: { fontSize: 10, fontFamily: "Inter-Bold" },
  txCard: { paddingHorizontal: 16, paddingVertical: 4, borderRadius: 16, borderWidth: 1 },
  empty: { textAlign: "center", paddingVertical: 40, fontSize: 14, fontFamily: "Inter-Medium" },
  photosGrid: { flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between" },
  photoItem: { width: "48%", marginBottom: 10, borderRadius: 16, overflow: "hidden", borderWidth: 1 },
  notesGrid: { flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between" },
  noteItem: { width: "48%", marginBottom: 8 },
  fab: { position: "absolute", bottom: 28, right: 20, flexDirection: "row", alignItems: "center", paddingHorizontal: 20, paddingVertical: 14, borderRadius: 28, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 10, elevation: 8, zIndex: 20 },
  fabLabel: { color: "#FFF", fontSize: 14, fontFamily: "Inter-Bold", marginLeft: 8 },
  emptyCard: { alignItems: "center", padding: 32, borderRadius: 20, borderWidth: 1, borderStyle: "dashed" },
  emptyIconWrap: { width: 56, height: 56, borderRadius: 28, alignItems: "center", justifyContent: "center", marginBottom: 14 },
  emptyTitle: { fontSize: 16, fontFamily: "Inter-Bold", marginBottom: 6 },
  emptySub: { fontSize: 12, fontFamily: "Inter-Medium", textAlign: "center", lineHeight: 18 },
  expSheetContent: { flex: 1, padding: 24 },
  expSheetTitle: { fontSize: 20, fontFamily: "Inter-Bold", textAlign: "center", marginBottom: 24 },
  expAmtWrap: { alignItems: "center", marginBottom: 24 },
  expAmtInput: { flexDirection: "row", alignItems: "center", borderRadius: 20, borderWidth: 1, paddingHorizontal: 20, paddingVertical: 12, width: 220 },
  expAmtSign: { fontSize: 28, fontFamily: "Inter-Bold", marginRight: 8 },
  expAmtText: { flex: 1, fontSize: 32, fontFamily: "Inter-Bold" },
  expLabel: { fontSize: 10, fontFamily: "Inter-Bold", textTransform: "uppercase", letterSpacing: 1, marginBottom: 6, marginLeft: 2 },
  expDescInput: { fontSize: 14, fontFamily: "Inter-Medium", padding: 16, borderRadius: 16, borderWidth: 1 },
  aiPredCard: { flexDirection: "row", alignItems: "center", alignSelf: "flex-start", paddingHorizontal: 14, paddingVertical: 8, borderRadius: 12, borderWidth: 1, marginBottom: 16 },
  aiPredIcon: { fontSize: 18, marginRight: 8 },
  aiPredCat: { fontSize: 12, fontFamily: "Inter-Bold" },
  aiPredLabel: { fontSize: 8, fontFamily: "Inter-Bold", textTransform: "uppercase", opacity: 0.7 },
  photoModalBg: { flex: 1, backgroundColor: "rgba(0,0,0,0.5)", justifyContent: "flex-end" },
  photoModalSheet: { borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, paddingBottom: 40 },
  photoModalHeader: { flexDirection: "row", justifyContent: "space-between", marginBottom: 20 },
  photoModalTitle: { fontSize: 20, fontFamily: "Inter-Bold" },
  photoModalClose: { width: 32, height: 32, borderRadius: 16, alignItems: "center", justifyContent: "center" },
  photoModalPreview: { width: "100%", height: 180, borderRadius: 16, marginBottom: 16 },
  photoModalInput: { fontSize: 14, fontFamily: "Inter-Medium", padding: 16, borderRadius: 14, borderWidth: 1, marginBottom: 16 },
  fullPhotoWrap: { flex: 1, backgroundColor: "#0F172A" },
  fullPhotoInfo: { position: "absolute", bottom: 48, left: 20, right: 20, backgroundColor: "rgba(255,255,255,0.1)", padding: 20, borderRadius: 20 },
  fullPhotoCaption: { color: "#FFF", fontSize: 16, fontFamily: "Inter-Bold", marginBottom: 4 },
  fullPhotoDate: { color: "rgba(255,255,255,0.5)", fontSize: 11, fontFamily: "Inter-Bold", textTransform: "uppercase", letterSpacing: 1 },
  ...
  noteSaveBtn: { backgroundColor: "#0F172A", paddingHorizontal: 20, paddingVertical: 10, borderRadius: 20 },
  noteSaveText: { color: "#FFF", fontSize: 13, fontFamily: "Inter-Bold" },
  noteBody: { flex: 1, paddingHorizontal: 28, paddingTop: 24 },
  noteTitleInput: { color: "#0F172A", fontSize: 26, fontFamily: "Inter-Bold", marginBottom: 16 },
  noteBodyInput: { color: "rgba(15,23,42,0.8)", fontSize: 16, fontFamily: "Inter-Medium", lineHeight: 28 },
  noteToolbar: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginHorizontal: 20, marginBottom: 36, backgroundColor: "rgba(255,255,255,0.6)", padding: 12, borderRadius: 20 },
  noteColors: { flexDirection: "row" },
  noteColorDot: { width: 28, height: 28, borderRadius: 14, marginHorizontal: 3, borderWidth: 2 },
  noteMoods: { flexDirection: "row", backgroundColor: "rgba(0,0,0,0.05)", padding: 3, borderRadius: 14 },
  noteMoodBtn: { width: 30, height: 30, borderRadius: 10, alignItems: "center", justifyContent: "center", marginHorizontal: 1 },
  noteMoodActive: { backgroundColor: "#FFF", shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 4, elevation: 2 },
  ...
  privacyIcon: { width: 34, height: 34, borderRadius: 10, alignItems: "center", justifyContent: "center", marginRight: 10 },
  privacyLabel: { fontSize: 14, fontFamily: "Inter-Bold" },
  privacySubLabel: { fontSize: 13, fontFamily: "Inter-Bold" },
  privacySub: { fontSize: 10, fontFamily: "Inter-Medium", marginTop: 1 },
  ...
  photoModalLabel: { fontSize: 14, fontFamily: "Inter-Bold" },
  ...
  fullPhotoPrivacy: { flexDirection: "row", alignItems: "center", backgroundColor: "rgba(255,255,255,0.1)", paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20, gap: 6 },
  fullPhotoPrivacyText: { color: "white", fontSize: 12, fontFamily: "Inter-Bold" },
  notePrivacyBtn: { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 16, marginRight: 10 },
  notePrivacyText: { color: "#0F172A", fontSize: 12, fontFamily: "Inter-Bold" },
});
